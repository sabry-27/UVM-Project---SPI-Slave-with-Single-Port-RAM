package WRAPPER_write_read_sequence_pkg;
    ...
    class WRAPPER_write_read_sequence extends uvm_sequence #(WRAPPER_sequence_item);
        ...
        task body();
            ...
            // Added: initialize first op
            seq_item.op = 3'b000;

            repeat (1000) begin
                start_item(seq_item);
                assert(seq_item.randomize()); // uses class constraints only
                finish_item(seq_item);

                // Added: update op for next iteration
